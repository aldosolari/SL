...
for (i in 1:length(levels(xcut))){
sub = (xcut==levels(xcut)[i])
lines(x[sub], fitted(lm(y[sub] ~ x[sub])) )
}
```
...
* __Regression splines__ are linear models with a spline basis
...
plot(x,y ,col="lightgray")
abline(v=knots, lty=2)
bs = splines::bs(x, degree=d, knots=knots, intercept=T)
for (i in 1:ncol(bs)) lines(x,bs[,i], col=i)
```
...
* The number of basis functions used by a natural cubic spline is
...
lines(x, fitted( lm(y ~ ns(x, knots=knots )) ) , col=i)
}
```
