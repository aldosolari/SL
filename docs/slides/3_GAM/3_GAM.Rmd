...
* The idea is that each predictor makes a separate contribution to the response, and these just add up
...
* To see why we need to do this, consider the simple case with $p = 2$. If we add constants $c_1$ to $f_1$ and $c_2$ to $f_2$, but subtract $c_1+c_2$ from $\beta_0$, then nothing observable has changed about the model
...
```{r}
library(RCurl)
calif <- read.csv("https://raw.githubusercontent.com/aldosolari/SL/master/data/calif.csv")
library(mgcv)
